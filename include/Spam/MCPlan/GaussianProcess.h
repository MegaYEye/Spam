...
/** Utily constants */
static const Real log2pi = std::log(TWO_PI);
...
public:
    ...
    /** Predict f_* ~ GP(x_*) */
    virtual Real f(const Vec3& xStar) {
	...
	virtual Real var(const Vec3& xStar) {
		...
		return (Real)cf->get(xStar, xStar) - v.dot(v); //cf->get(x_star, x_star) - v.dot(v);
    }
...
protected:
	...
	//			context.write("GP::compute(): Computing k(%lu, %lu)\r", i, j);
		...
		// atlas computation
		if (desc.atlas) {
			const Eigen::MatrixXd invKpp = L->inverse();
        ...
        alpha_needs_update = true;
        context.write("GP::Compute(): Elapsed time: %.4fs\n", (float)(clock() - t)/CLOCKS_PER_SEC);
    }
